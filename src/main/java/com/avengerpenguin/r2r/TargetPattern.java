...
import com.avengerpenguin.r2r.functions.HelperFunctions;
import com.avengerpenguin.r2r.parser.TargetPatternLexer;
...
import com.avengerpenguin.r2r.functions.HelperFunctions;
import com.avengerpenguin.r2r.parser.*;
import com.avengerpenguin.r2r.parser.TargetPatternParser.targetPattern_return;
import com.hp.hpl.jena.datatypes.TypeMapper;
import com.hp.hpl.jena.datatypes.RDFDatatype;

import com.avengerpenguin.r2r.TripleElement.Type;
...
public class TargetPattern implements Serializable {
	...
	private void addObjectsToStatement(Resource subject, Property property, TripleElement object, VariableResults results, Model model, int blankNodeGroup) {
		...
		TripleElement.Type type = object.getType();
		if(type== TripleElement.Type.IRI || type== TripleElement.Type.IRIVARIABLE) {
		...
		else if(type== TripleElement.Type.DATATYPESTRING || type== TripleElement.Type.DATATYPEVARIABLE) {
			List<String> values = null;
			if(type== TripleElement.Type.DATATYPEVARIABLE)
		...
		else if(type== TripleElement.Type.STRING || type== TripleElement.Type.STRINGVARIABLE) {
			List<String> values = null;
			if(type== TripleElement.Type.STRINGVARIABLE)
		...
		else if(type== TripleElement.Type.LANGTAGSTRING || type== TripleElement.Type.LANGTAGVARIABLE) {
			List<String> values = null;
			if(type== TripleElement.Type.LANGTAGVARIABLE)
				values = results.getResults(object.getValue(0));
			else {
				values = new ArrayList<String>();
				values.add(object.getValue(0));
		...
		else if (type== TripleElement.Type.VARIABLE) {
	...
	private List<String> getIriValuesOfTripleElement(TripleElement element, VariableResults results) {
		...
		if(element.getType()== TripleElement.Type.IRI) {
	...
	private List<Resource> getSubjectValues(TripleElement element, VariableResults results, Model model, int blankNodeGroup) {
		...
		TripleElement.Type type = element.getType();
		
		if(type== TripleElement.Type.IRI) {
			...
			if(type== TripleElement.Type.VARIABLE) {
